...
const readFacebookAppSecret = (): string => {
  try {
...
const normalizeInstagramInputUrl = (raw: string): string => {
  const trimmed = raw.trim();
  const withProtocol = /^https?:\/\//i.test(trimmed) ? trimmed : `https://${trimmed}`;
  return withProtocol;
};

const validateResolvableInstagramUrl = (url: string): { ok: true; normalized: string } | { ok: false; reason: string } => {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
...
const inferMediaType = (normalizedUrl: string, html?: string): "IMAGE" | "VIDEO" | "CAROUSEL_ALBUM" => {
...
const fetchAppAccessToken = async (): Promise<string | null> => {
...
const fetchInstagramOEmbed = async (normalizedUrl: string, accessToken: string): Promise<Record<string, unknown>> => {
...
export const mediaGateway = onRequest(
  ...
  async (request, response) => {
    ...
    const userToken =
      typeof integrationSnap.data()?.accessToken === "string" ? (integrationSnap.data()?.accessToken as string).trim() : "";
    ...
    const tryResolveWithToken = async (accessToken: string): Promise<Record<string, unknown>> =>
      fetchInstagramOEmbed(normalized, accessToken);
