...
import { safePayloadMeta, validateGatewayPayloadForFlow } from "./aiGatewayPayloadGuard.js";
if (getApps().length === 0) {
    initializeApp();
}
...
const flowOutputContracts = {
    ...
    local_scenario_chat: "Return JSON {\"assistantMessage\":\"...\",\"updatedScenario\": optional full LocalScenario}. Preserve scenario id from payload when returning updatedScenario. When the user asks to replace, remove, reorder, or add a stop, return updatedScenario with revised blocks and coherent times; otherwise omit updatedScenario and only reply in assistantMessage.",
    trip_chat_replan: "Return JSON {\"assistantMessage\":\"...\",\"structuredPatchSummary\":\"...\",\"proposal\": optional ReplanProposal}. If proposing edits, proposal must include id,userId,tripId,createdAt,reason,summary,actions. Actions must be move_activity, remove_activity, replace_activity, or compress_day.",
    trip_revalidation: "Return JSON {\"assistantMessage\":\"...\",\"structuredPatchSummary\":\"...\",\"proposal\": optional ReplanProposal}. Explain tradeoffs from supplied warnings and provider facts only.",
    unfinished_day_recovery: "Return JSON {\"assistantMessage\":\"...\",\"structuredPatchSummary\":\"...\",\"proposal\": ReplanProposal}. Preserve must/locked items and completion history.",
    preference_learning: "Return JSON {\"assistantMessage\":\"...\",\"structuredPatchSummary\":\"...\"}. Suggest preference learning from completion and skipping patterns without modifying data directly.",
    post_trip_analysis: "Return JSON {\"structuredInsights\":[{\"key\":\"...\",\"detail\":\"...\",\"weight\":\"low|medium|high\"}],\"followUpActions\":[{\"id\":\"...\",\"label\":\"...\",\"kind\":\"preference|trip_style|logistics|other\"}],\"summaryForUi\":\"...\"}. summaryForUi max 2000 chars. No other top-level keys.",
    live_decision_support: "Return JSON {\"statusLine\":\"...\",\"rationaleBullets\":[\"...\"],\"suggestedUiActions\":[\"continue\"|\"skip_next_low_priority\"|\"shorten_current_item\"|\"reorder_remaining\"|\"end_day\"|\"open_day_editor\"],\"confidence\":\"low|medium|high\"}. statusLine max 400 chars. No other top-level keys.",
};
const createSystemPrompt = (flow) => [
...
const resolveFlowFromPath = (path, fallback) => {
...
export const aiGateway = onRequest({
    ...
    const payloadGuard = validateGatewayPayloadForFlow(flow, parsed.data.payload);
    ...
    logger.info("aiGateway.request_accepted", {
        flow,
        meta: safePayloadMeta(flow, parsed.data.payload),
    });
